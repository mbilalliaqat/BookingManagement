...
const GamcaTokenRemainingPay = ({ gamcaTokenId, onPaymentSuccess }) => {
    ...
    useEffect(() => {
        if (gamcaTokenDetails) {
            ...
            const originalRemaining = parseFloat(gamcaTokenDetails.remaining_amount) || 0;
            const updatedRemaining = originalRemaining - totalPayment;
            
            setCurrentRemainingAmount(updatedRemaining);
        }
    }, [gamcaTokenDetails, newPayment.paid_cash, newPayment.paid_in_bank]);
    ...
    const fetchGamcaTokenDetails = async () => {
        try {
            ...
            if (response.data) {
                ...
                setCurrentRemainingAmount(parseFloat(response.data.gamcaToken.remaining_amount) || 0);
            }
        } catch (error) {
                    ...
                    <div className="bg-white rounded-lg p-6 w-96">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className={`p-4 rounded-lg border-2 ${
                                ...
                                {currentRemainingAmount === 0 && (parseFloat(newPayment.paid_cash) || 0) + (parseFloat(newPayment.paid_in_bank) || 0) > 0 && (
